...
export default function TechStackPage() {
  ...
  return (
    ...
    >
        ...
        <div className="flex flex-wrap items-center justify-center gap-1.5 sm:gap-2 mt-6 sm:mt-8 max-w-full px-1">
          {categories.map((cat) => (
            <button
              ...
              className={`px-3 py-1.5 sm:px-4 sm:py-2 rounded-full text-[11px] sm:text-xs font-semibold tracking-wide transition-all cursor-pointer ${
      ...
      <div className="mb-12">
        <div className="p-4 sm:p-8 md:p-10 rounded-2xl border border-white/18 shadow-2xl relative bg-[#010101]/60 backdrop-blur-[2px]">
              ...
              return (
                ...
                >
                  ...
                  <LiquidGlassWrapper
                    ...
                    className={`p-2.5 sm:p-4 rounded-xl border transition-all duration-300 cursor-pointer flex flex-col items-center justify-center text-center gap-1.5 sm:gap-2 group h-full ${
                      isSelected
                        ? 'border-[#00E676]/70 bg-white/10 shadow-[0_0_24px_rgba(0,230,118,0.35)]'
                        : 'border-white/12 hover:border-[#00E676]/40 bg-white/5'
                    }`}
                  >
                    <div className="p-2 sm:p-2.5 rounded-lg bg-[#010101]/80 border border-white/12 group-hover:scale-110 transition-transform">
                      <Icon className={`w-5 h-5 sm:w-6 sm:h-6 ${skill.color}`} />
                    </div>
                    <div className="font-semibold text-xs sm:text-sm text-white group-hover:text-[#00E676] transition-colors truncate w-full">
                      {skill.name}
                    </div>
          ...
          {activeSkill && (
            <div className="mt-6 sm:mt-8 pt-6 border-t border-white/12 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 sm:gap-6 bg-[#010101]/80 p-4 sm:p-6 rounded-xl border border-white/15">
              <div className="flex items-start sm:items-center gap-3 sm:gap-4">
                <div className="p-2.5 sm:p-3.5 rounded-xl bg-[#00E676]/10 border border-[#00E676]/30 shrink-0">
                  <activeSkill.icon className={`w-6 h-6 sm:w-7 sm:h-7 ${activeSkill.color}`} />
                </div>
                <div>
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <span className="px-2 sm:px-2.5 py-0.5 rounded-full text-[11px] sm:text-xs font-semibold text-[#00E676] bg-[#00E676]/15 border border-[#00E676]/30">
                      {activeSkill.level}
                    </span>
                  </div>
                  ...
                  <p className="text-[11px] sm:text-xs text-slate-400 mt-1.5 flex items-start sm:items-center gap-1.5 leading-relaxed break-words">
                    <Terminal className="w-3.5 h-3.5 text-[#00E676] shrink-0 mt-0.5 sm:mt-0" />
                    <span>{activeSkill.architectureDetails}</span>
                  </p>
                </div>
              </div>

              <div className="shrink-0 self-start md:self-auto flex items-center gap-1.5 sm:gap-2 text-[11px] sm:text-xs text-[#00E676] font-semibold bg-[#00E676]/10 px-3 sm:px-4 py-1.5 sm:py-2 rounded-xl border border-[#00E676]/30 shadow-sm">
